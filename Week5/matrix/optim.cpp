...
matrix operator-(const double t, const matrix& first){
    matrix temp(first.rows,first.cols);
    temp = first*-1;
    return temp+t;
}
...
matrix operator/(const double t, const matrix& first){
    ...
    for(unsigned long k=0; k<size-(size%4);k+=4){
        element_operation=_mm256_loadu_pd(&first.data[k]);
        element_operation = _mm256_div_pd(constant_operation,element_operation);
        _mm256_storeu_pd(&quotient.data[k], element_operation);
    }
    for(unsigned long l=size-(size%4);l<size;l++){
        quotient.data[l]=t/first.data[l];
    }
    return quotient;  
}
...
matrix& operator+=(matrix& self,const matrix& other){
    ...
    uint64_t size = self.rows*self.cols;
    __m256d current_self, current_other;
    for(uint64_t i=0;i<size-(size%4);i+=4){
        current_self = _mm256_loadu_pd(&self.data[i]);
        current_other = _mm256_loadu_pd(&other.data[i]);
        current_self = _mm256_add_pd(current_self,current_other);
        _mm256_storeu_pd(&self.data[i],current_self);
    }
    for(uint64_t j=size-(size%4);j<size;j++){
...
matrix& operator-=(matrix& self,const matrix& other){
    ...
    for(uint64_t i=0;i<size-(size%4);i+=4){
        current_self = _mm256_loadu_pd(&self.data[i]);
        current_other = _mm256_loadu_pd(&other.data[i]);
        current_self = _mm256_sub_pd(current_self,current_other);
        _mm256_storeu_pd(&self.data[i],current_self);
    }
    for(uint64_t j=size-(size%4);j<size;j++){
        self.data[j]-=other.data[j];
...
matrix& operator*=(matrix& self,const matrix& other){
    ...
    for(uint64_t i=0;i<size-(size%4);i+=4){
        current_self = _mm256_loadu_pd(&self.data[i]);
        current_other = _mm256_loadu_pd(&other.data[i]);
        current_self = _mm256_mul_pd(current_self,current_other);
        _mm256_storeu_pd(&self.data[i],current_self);
    }
    for(uint64_t j=size-(size%4);j<size;j++){
        self.data[j]*=other.data[j];
...
void mlt_log(matrix &m1, matrix &m2, unsigned long start, unsigned long end){
        if(start==end) {
            return;
        }
        for(unsigned long i=start;i<end;i++){
            m2.data[i]=log(m1.data[i]);
        }
        return;
}
...
matrix log(matrix &a) {
    ...
    for(unsigned long j=0;j<MAX_THREADS-1;j++){
        unsigned long start=j*(no_elements/MAX_THREADS);
        unsigned long end=(j+1)*(no_elements/MAX_THREADS);
        T[j]=thread(mlt_log, ref(a), ref(res), start,end);
    }
    T[MAX_THREADS-1]= thread(mlt_log, ref(a), ref(res),(MAX_THREADS-1)*(no_elements/MAX_THREADS) ,no_elements);
    for(int k=0;k<MAX_THREADS;k++){
        T[k].join();
    }
...
double dot(const matrix& first, const matrix& second){
    ...
    double d = 0;
    __m256d vect_1, vect_2, temp_add;
    double add_temp[4];
    for (uint64_t count=0; count<n-(n%4);count+=4){
        vect_1 = _mm256_loadu_pd(&first.data[count]);
        vect_2 = _mm256_loadu_pd(&second.data[count]);
        temp_add = _mm256_mul_pd(vect_1,vect_2);
        temp_add = _mm256_hadd_pd(temp_add,temp_add);
        _mm256_storeu_pd(add_temp,temp_add);
        d+=(add_temp[0]+add_temp[1]);
    }
    for(uint64_t count =n-(n%4);count<n;count++){
        d+=first.data[count]*second.data[count];
    }
    return d;
}
...
double norm(const matrix& v){
    ...
    __m256d temp_vec;
    double vec_temp[4];
    for(uint64_t counter=0;counter<size-(size%4);counter+=4){
        temp_vec= _mm256_loadu_pd(&v.data[counter]);
